...
public class RegularHebrewDate extends HebrewDate {
    ...
    private static final String[] DAYS = {"ראשון", "שני", "שלישי", "רביעי", "חמישי", "שישי", "שבת"};
    ...
    public static String getParashaEng(int parashaNum) {
        String parasha = "";
        try {
            parasha = parshiosEng[parashaNum];
        } catch (Exception e) {
    ...
    public static String getParashaAsString(int parashaNum) {
        if (parashaNum > 52) {
            return "פרשיות " + getParasha(parashaNum);
        }
        return "פרשת " + getParasha(parashaNum);
    }
    ...
    public static String getCandleAndHavdala() {
        StringBuilder sb = new StringBuilder("פרשת השבוע:");
        Calendar c = Calendar.getInstance();
        ...
        sb.append(cl.getCandleLighting()).append(", ערב שבת:");
        while ("".equals(cl.getHavdala())) {
            c.add(Calendar.DAY_OF_MONTH, 1);
            cl = new CandleLighting(new HebrewDate(c));
        }
        sb.append(cl.getHavdala()).append(" מוצאי ש").append(cl.getDefaultLocale().getName());
        return sb.toString();
    }
    ...
    public String getHoliday() {
        switch (hebrewMonth) {
            case 1:
                if (hebrewDate == 14) return "ערב פסח";
                if (hebrewDate == 15 || hebrewDate == 21 || (!israeli && (hebrewDate == 16 || hebrewDate == 22))) return "פסח";
                if (hebrewDate >= 17 && hebrewDate <= 20 || (hebrewDate == 16 && israeli)) return "חול המועד פסח";
                break;
            case 2:
                if (hebrewDate == 14) return "פסח שני";
                break;
            case 3:
                if (hebrewDate == 5) return "ערב שבועות";
                if (hebrewDate == 6 || (hebrewDate == 7 && !israeli)) return "שבועות";
                break;
            case 4:
                if ((hebrewDate == 17 && day != 7) || (hebrewDate == 18 && day == 1)) return "יז בתמוז";
                break;
            case 5:
                if ((day == 1 && hebrewDate == 10) || (day != 7 && hebrewDate == 9)) return "תשעה באב";
                if (hebrewDate == 15) return "טו באב";
                break;
            case 6:
                if (hebrewDate == 29) return "ערב ראש השנה";
                break;
            case 7:
                if (hebrewDate == 1 || hebrewDate == 2) return "ראש השנה";
                if ((hebrewDate == 3 && day != 7) || (hebrewDate == 4 && day == 1)) return "צום גדליה";
                if (hebrewDate == 9) return "ערב יום כיפור";
                if (hebrewDate == 10) return "יום כיפור";
                if (hebrewDate == 14) return "ערב סוכות";
                if (hebrewDate == 15 || (hebrewDate == 16 && !israeli)) return "סוכות";
                if (hebrewDate >= 17 && hebrewDate <= 20 || (hebrewDate == 16 && israeli)) return "חול המועד סוכות";
                if (hebrewDate == 21) return "הושענא רבא";
                if (hebrewDate == 22) return "שמיני עצרת";
                if (hebrewDate == 23 && !israeli) return "שמחת תורה";
                break;
            case 9:
                if (hebrewDate == 24) return "ערב חנוכה";
                if (hebrewDate >= 25) return "חנוכה";
                break;
            case 10:
                if (hebrewDate == 1 || hebrewDate == 2 || (hebrewDate == 3 && isKislevShort())) return "חנוכה";
                if (hebrewDate == 10) return "עשרה בטבת";
                break;
            case 11:
                if (hebrewDate == 15) return "טו בשבט";
                break;
            case 12:
            case 13:
                // טיפול בפורים (כולל שנה מעוברת)
                if ((hebrewMonth == 12 && !isHebrewLeapYear()) || hebrewMonth == 13) {
                    if (((hebrewDate == 11 || hebrewDate == 12) && day == 5) || (hebrewDate == 13 && !(day == 6 || day == 7))) return "תענית אסתר";
                    if (hebrewDate == 14) return "פורים";
                    if (hebrewDate == 15) return "פורים שושן";
                } else if (hebrewMonth == 12 && isHebrewLeapYear()) {
                    if (hebrewDate == 14) return "פורים קטן";
                }
                break;
        }
        return "";
