...
public class HebrewDate implements Comparable, Cloneable {
    ...
    public static String gimatria(int n) {
        ...
        if (n % 1000 == 0) {
            ...
            // הערה: בדרך כלל נהוג להוסיף גרש אחרי האלפים, למשל ה'תשפ"ד
        ...
        if (n >= 100) {
            p.append("קרא".charAt(n / 100 - 1));
            n %= 100;
        }

        if (n >= 10) {
            // תיקון מובנה עבור יה-יו לט-טו
            if ((n == 15) || (n == 16)) {
                n -= 9;
            }
            ...
            if (n / 10 > 0) {
                ...
                // טיפול במקרה של ט"ו/ט"ז אחרי ההפחתה
                p.append('ט');
            }
            n %= 10;
        }
        ...
        // הוספת גרשיים או גרש לפורמט תקני
        if (p.length() <= 1) {
            p.append('\'');
        } else {
            p.insert(p.length() - 1, '"');
    ...
    public String getHebrewDateAsHebStringWithoutYear() {
        return gimatria(hebrewDate) + " ב" + getHebrewMonthAsString();
    }

    public String getHebrewDateAsHebStringWithoutYear2() {
        return gimatria(hebrewDate) + ' ' + getHebrewMonthAsString();
    }
    ...
    public int compareTo(Object o) {
        ...
        return Integer.compare(absDate, hebDate.absDate);
    }
